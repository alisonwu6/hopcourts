      ...
      <div className="rounded-3xl border border-slate-100 bg-white p-7 shadow-xl shadow-slate-200/50">
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl border border-indigo-100/50 bg-indigo-50 shadow-inner">
          ...
          <div className="hidden flex-col items-end text-right md:flex">
            ...
            <div className="rounded-full border border-indigo-100/30 bg-indigo-50/50 px-2 py-0.5 text-[10px] font-extrabold uppercase tracking-tight text-indigo-500">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          <div>
            ...
            <select
              ...
              className="w-full rounded-xl border border-slate-200/50 bg-slate-50 px-3 py-2 text-xs font-black tracking-tight text-slate-700 outline-none transition-all hover:border-slate-300 focus:border-indigo-500"
            >
              {SPORTS.map((s) => (
          ...
          <div>
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-200/50 bg-slate-50 px-3 py-2 text-xs font-black tracking-tight text-slate-700 focus:border-indigo-500"
            />
          </div>
          <div>
            <label className="mb-1.5 ml-1 block text-[10px] font-black uppercase tracking-widest text-slate-400">
        ...
        <div className="mt-6 border-t border-slate-50 pt-6">
          <p className="max-w-2xl text-[11px] font-semibold leading-relaxed text-slate-400">
            <span className="mr-2 text-[9px] font-bold uppercase tracking-widest text-indigo-600">
              Smart Generation:
            </span>
        ...
        <div className="flex border-b border-slate-50 bg-slate-50/30 p-2">
          {DAYS.map((day) => (
            <button
              ...
              className={`flex-1 rounded-2xl py-3 text-[10px] font-black uppercase tracking-widest transition-all ${activeDay === day.key ? 'bg-white text-indigo-600 shadow-lg shadow-indigo-100 ring-1 ring-slate-100' : 'text-slate-400 hover:text-slate-600'}`}
            >
              {day.label}
            </button>
          ))}
          ...
          <div className="space-y-4">
            {slots
              ...
              .map((slot) => (
                <div
                  key={slot.id}
                  className="rounded-[1.5rem] border border-slate-100 bg-slate-50/50 p-6 transition-all duration-300 hover:border-indigo-100 hover:bg-white hover:shadow-xl hover:shadow-indigo-50/50"
                >
                    ...
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                      <div>
                        ...
                        <select
                          ...
                          className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-black text-slate-700 shadow-sm transition-colors focus:border-indigo-500"
                        >
                          {SPORTS.map((s) => (
                            <option key={s}>{s}</option>
                          ))}
                    ...
                    <div className="grid grid-cols-1 gap-6 border-t border-slate-100/80 pt-5 md:grid-cols-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-black text-indigo-600 shadow-sm"
                        />
                      </div>
                    </div>
                  </div>
            ...
            {slots.filter((s) => s.day_of_week === activeDay).length === 0 && (
              <div className="flex flex-col items-center rounded-[2rem] border-2 border-dashed border-slate-100/50 bg-slate-50/30 py-20 text-center">
                ...
                <button
                  onClick={handleAddSlot}
                  className="mt-2 text-[10px] font-black uppercase tracking-[0.2em] text-indigo-600 hover:underline"
                >
                  {' '}
                  Create First Slot{' '}
                </button>
