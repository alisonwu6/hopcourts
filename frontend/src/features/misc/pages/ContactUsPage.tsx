...
import { ChevronRight, Send, CheckCircle2, AlertCircle, Lightbulb, UserCircle2, HelpCircle, Check } from 'lucide-react'
      ...
      <main className="mx-auto max-w-2xl px-4 py-6">
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="space-y-3">
            ...
            <div className="mt-2 relative rounded-2xl bg-white p-4 shadow-sm border border-slate-200 focus-within:border-emerald-500 focus-within:ring-1 focus-within:ring-emerald-500 transition-all">
              <textarea
                id="content"
                value={content}
                onChange={(e) => setContent(e.target.value)}
                placeholder={getPlaceholder(type)}
                className="h-48 w-full resize-none border-none bg-transparent p-0 text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-0 text-base leading-relaxed"
                required
              />
            </div>
          </div>
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-full px-6 py-4 text-lg font-bold shadow-sm transition-all active:scale-[0.98] disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-400 disabled:shadow-none bg-emerald-600 text-white hover:bg-emerald-700"
          >
            {isSubmitting ? (
              '傳送中...'
            ) : (
