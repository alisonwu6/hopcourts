...
export function OnboardingPage() {
  ...
  const sportsDisplayCount = sports.includes('I am just getting started') ? 0 : sports.length
    ...
    >
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-4">
        <div className="flex flex-col gap-3">
          ...
          <div className="text-sm font-semibold text-slate-500">{currentStep}</div>
        ...
        {currentStep === 'Sports' && (
          <div className="space-y-3">
            <div className="text-sm font-semibold text-slate-500">{sportsDisplayCount}/3 selected</div>
            {sports.length > 0 && (
              ...
              {filteredSports.map((sport) => {
                ...
                return (
                  <button
                    ...
                    style={
                      selected
                        ? { background: withAlpha(accent.ring, 0.14), boxShadow: `0 8px 20px -14px ${accent.ring}` }
                        : undefined
                    }
                  >
                    <span className="text-sm font-semibold text-slate-900">{sport}</span>
        ...
        {currentStep === 'Trying' && (
          <div className="space-y-3">
            <div className="text-sm font-semibold text-slate-500">{tryingDisplayCount}/2 selected</div>
            {trying.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {trying.map((item) => (
                  <span
        ...
        {currentStep === 'Bio' && (
          <div className="space-y-4">
            ...
            />
            <div className="text-right text-xs text-slate-500">{bio.length}/120</div>
            <button
              type="button"
              className="inline-flex items-center gap-2 text-sm font-semibold text-blue-600 hover:underline"
              onClick={() =>
                setBio('I move to feel grounded and find my crew wherever I land.')
              }
            >
              Generate for me
            </button>
