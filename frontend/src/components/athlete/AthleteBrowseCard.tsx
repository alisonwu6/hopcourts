...
function resolveSport(athlete: AthleteCardProps) {
  return athlete.sport ?? athlete.primarySport ?? 'Sport TBD'
}

function resolveVibes(athlete: AthleteCardProps) {
  return athlete.vibes ?? athlete.tags ?? []
}

export const AthleteBrowseCard = memo(({ athlete }: { athlete: AthleteCardProps }) => {
  const status = formatStatus(athlete)
  const vibes = resolveVibes(athlete)
  const displaySport = resolveSport(athlete)
  const highFives = athlete.highFiveCount

  return (
    <article className="overflow-hidden rounded-2xl border border-[#E6E6E6] bg-white shadow-[0_4px_12px_rgba(0,0,0,0.05)]">
      <div className="relative h-48 w-full overflow-hidden">
        <img
          src={athlete.coverUrl ?? athlete.backgroundUrl ?? fallbackHero}
          onError={(event) => {
        ...
        <div className="absolute left-4 top-4 flex items-center gap-3">
          ...
          <div className="space-y-0.5 text-white drop-shadow-sm">
            ...
            <div className="text-sm text-white/80">{displaySport}</div>
          </div>
        </div>
        <button
          type="button"
      ...
      <div className="space-y-3 px-4 py-4">
        ...
        {typeof highFives === 'number' && (
          <div className="text-xs text-[#6E6E6E]">High-fives received · {highFives}</div>
        )}
      </div>
    </article>
  )
